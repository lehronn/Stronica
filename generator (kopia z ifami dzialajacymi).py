...
def htmlGen(filename, filePath, fileType, meta, header, nav, aside, content, footer):
    ...
    #tworzenie pliku
    if fileType == "site":
        os.mknod(public_texts_files_path + "/" + str(filename)) ################te oba ify są do poprawy bo pliki tworzą się w jednym katalogu a powinny w różnych, ścieżka prawidłowa jest w zmiennej public_file_path
        print("prawidłowa ścieżka gdzie powinno dodać nowe pliki dla strony:")
        print(public_file_path) 
    elif fileType == "category":
        print("prawidłowa ścieżka gdzie powinno dodać nowe pliki dla categorrii:")
        print(public_file_path) 
        if not os.path.dirname(filePath): os.makedirs((os.path.dirname(filePath)))
        os.mknod(os.path.dirname(filePath) + "/" + filename) ############################################
    #zlepianie części składowych w jedną zmienną i zapis do pliku
    with open(public_texts_files_path + "/" + filename,'wt') as f:
        f.write(meta)
        f.write(header)
        f.write(nav)
        f.write(aside)
...
htmlGen("index.html", public_path, "", meta, header, nav, aside, content, footer)
